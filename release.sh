...
STANDALONE_DEST="../firebase-clients/libs/backfire"
...
git tag v$VERSION
if [[ $? -ne 0 ]]; then
  echo "Error: Failed to do 'git tag' from backfire repo."
  exit 1
fi
...
git push --tags
if [[ $? -ne 0 ]]; then
  echo "Error: Failed to do 'git push --tags' from backfire repo."
  exit 1
fi
